...
$classes = 'post-masonry post-layout-blocks grid-height __' . ($post_format ? $post_format : 'standard') . (wp_is_mobile() ? ' __mobile' : '');
...
if ($content_width == 'expanded') {
	if ($sidebar_location) {
		$col_classes[] = 'col-xs-12 col-md-6 col-xxl-4';
	} else {
...
if (is_sticky() || ThewriterOptions::get('single_post--double_width')) {

    if ($content_width == 'expanded') {
        if ($sidebar_location) {
            $col_classes[] = 'col-xs-12 col-md-12 col-xxl-8';
        } else {
            $col_classes[] = '';
        }
    } elseif ($content_width == 'normal') {
...
?>
		<article id="post-<?php the_ID(); ?>" <?php post_class( $classes ); ?> <?php if($thewriter_custom_post_padding != 'none') {?> style="padding-left: <?php echo esc_attr($thewriter_custom_post_padding)?>px!important; padding-right: <?php echo esc_attr($thewriter_custom_post_padding)?>px!important" <?php }?>>
			<?php get_template_part( 'post-templates/blocks/content', $post_format ); ?>
		</article>
